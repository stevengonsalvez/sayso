...
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw dictionary for every entry; ships ExecutionTelemetry and .timedOut; timestamped backup capped at 3 files on whole-file JSON syntax failure |
...
| Candidate Element Verification in execute() | Post-approval verification scoped to target element, not whole window | For candidate actions, execute() re-asserts target element identity (candidateID, title, isEnabled, !isProtected) on the frontmost window rather than comparing the full-window snapshot fingerprint, preventing live-updating ambient window controls (chat feeds, clocks, timers) from causing false staleTarget rejections after confirmation pauses |
...
| Layer | Scope | Gate |
|---|---|---|
| Unit | Menu hierarchy step parsing | "menu File > Save then click Confirm" splits on " then " (case-insensitive) into 2 steps while preserving ">" within the menu path. Segment quoting syntax: path segments containing " then " or special delimiters must be individually quoted (e.g. menu File > "Export then Print") |
...
| Unit | Candidate element check in execute() | Live-updating ambient window content does not trigger staleTarget; only target candidateID, title, or state mismatch aborts execution |
...
| Unit | Resilient audit journal decoding | Corrupt or unknown action entries preserved without wiping 500-entry journal; timestamped backup control-audit.json.corrupt-<timestamp> (capped at 3 files) written on whole-file JSON syntax corruption |
