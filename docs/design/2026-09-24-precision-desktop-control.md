...
| ControlPolicy | SaysoCore | Fail-closed destructive classification and review gating | Formalized invariant: compiler eliminates default: fallbacks; DesktopAction.isDestructive for .menu checks path.contains(where: { isDestructiveControlTitle($0) }) for intrinsic reporting; ControlPolicy.requiresConfirmation gates unlisted menus (!safeMenuAllowlist.contains(path)) or destructive titles; ControlPolicy.confirmationTier(step) returns .autoRun only when canAutoRun(step) is true (confidence >= 0.60, non-nil title, not requiresConfirmation), otherwise categorizes into .physical (named physicalConfirmationWords, .key, .clickAt, unlisted destructive words) or .spokenOrPhysical (spokenConfirmationWords, unlisted menus); destructive check wins |
...
| Confirmation Tiers & Banner Safety | Single ControlPolicy.confirmationTier function | ControlPolicy.confirmationTier(step) maps actions to 3 tiers: (1) .physical: .clickAt, .key, and candidate/menu titles matching ControlPolicy.physicalConfirmationWords (pay, purchase, transfer, send, delete, erase, trash, empty, quit, reset, revoke, deactivate, uninstall, remove, clear, discard, and any unclassified destructive word) require physical Return/Space key (captured via local NSEvent monitor on NotchHUD without stealing focus) or pointer click on banner; (2) .spokenOrPhysical: benign review actions (unlisted menus, sensitive toggles, and words in ControlPolicy.spokenConfirmationWords: cancel, close, submit, post, share, reply, forward, archive, publish, order, book, resend, unsend) can be confirmed by spoken "confirm action" / "approve action" under active AEC or by physical Return/click; (3) .autoRun: returns .autoRun only when canAutoRun(step) evaluates true (requiring confidence >= 0.60, !step.requiresConfirmation, non-nil candidateTitle, and isDestructiveControlTitle == false for .press / .select, or allowlisted menu path without destructive segments); any step with confidence < 0.60, step.requiresConfirmation == true, or nil candidateTitle falls back to confirmation review. Dictation insertion into target app is suppressed while banner is active |
| Headless Action Confirmation | Headless callers require step-bound approval; .physical barred headless | For automated CLI and socket calls, any action where confirmationTier(step) == .physical (including .clickAt, .key, and physical destructive words) is barred headless and returns reviewRequired error JSON directing user to Notch HUD. Benign review actions (.spokenOrPhysical) require explicit step-bound approval specifying stepId previously returned in reviewRequired response (sayso control approve --step-id <UUID>), preventing blind batch approval |
...
| Unit | Headless destructive action approval | CLI and socket control calls return reviewRequired error JSON for gated steps; all .physical steps (including .key and .clickAt) are barred headless; benign review actions execute only with explicit step-bound approval (sayso control approve --step-id <UUID>) |
...
| Unit | Confirmation banner safety | Physical Return/Space key (local NSEvent monitor) or pointer click required for all .physical actions (.clickAt, .key, physical destructive words); spoken "confirm action" / "approve action" / "cancel action" / "abort action" accepted under active AEC for benign review actions; bare words rejected |
