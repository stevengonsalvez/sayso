...
| Hands-Free User | Disambiguate without mouse | Spoken badge selection ("badge 2") and spoken confirmation for .spokenOrPhysical review tier; confirms .physical tier via macOS Accessibility (Voice Control, Switch Control, Dwell Click) |
...
|---|---|---|---|
| ControlPlanner | SaysoCore | Sub-15ms closed-vocab intent parsing alongside LLM fallback | Extended: spoken grammar parses menu hierarchy using "choose" (e.g. "menu File choose Save"); CLI and automation socket use ">"; empty path [] and depth > 3 reject fail-closed with invalidMenuPath |
...
| NotchHUD | SaysoNotch | Visual reticle, badge overlay, and telemetry breakdown | Extended: spatial badge rendering on nonactivating panel; all .physical actions (.quit, .clickAt, .key, physical destructive words) require pointer click on HUD banner with genuine HID hardware event state (no synthetic CGEvent self-approval, no key capture on non-key panel, avoiding focus stealing and target app default button activation); barred from headless self-approval; benign review actions allow spoken confirm/cancel banner with 5s/15s VAD timeout under active AEC or pointer click |
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw JSON values (dictionaries, arrays, strings, numbers, nulls) for every entry; append aborts fail-closed on read error without overwrite, retaining best-effort in-memory entries and displaying HUD alert banner; writes control-audit.version sidecar strictly monotonically (never lowers schemaVersion on disk: newVersion = max(existingVersion, 1)) after journal is written, using atomic temp-file rename ordering; older builds encountering schemaVersion > 1 treat entries as raw JSON values, preserve existing schemaVersion, and append safely; if backup write fails on corrupt parse, aborts append fail-closed; ships ExecutionTelemetry, ControlEffect.timedOut, and ControlSessionStepResult.timedOut; retains initial corrupt backup and up to 2 timestamped backups (capped at 3 files) on whole-file JSON syntax failure (see Safety Invariants) |
...
| Menu Traversal & Leaf Check | 45ms (3x 15ms p50) | 750ms (3x 250ms) | Depth <= 3 AXMenuBar item traversal. If any level times out, traversal aborts fail-closed, performs kAXCancelAction or kAXPressAction to dismiss open menus, and reports .timedOut |
...
| Total (Benign Element Fast-Path) | ~71ms | ~2.5s | Fast-path p50 target ~71ms (8ms parse + 20ms capture #1 + <1ms pre-flight + 4ms dispatch + 20ms capture #2 + 15ms diff + ~4ms telemetry); worst-case bounded at ~2.5s (15ms + 750ms + <1ms + 250ms + 1450ms), accounting for capture #2 overlapping with the post-assert diff ceiling |
| Total (Benign Menu Fast-Path) | ~116ms | ~3.2s | Fast-path p50 target ~116ms (8ms parse + 20ms capture #1 + <1ms pre-flight + 45ms menu traversal + 4ms dispatch + 20ms capture #2 + 15ms diff + ~4ms telemetry); worst-case bounded at ~3.2s (15ms + 750ms + <1ms + 750ms menu traversal + 250ms dispatch + 1450ms diff ceiling) |
...
| .effectUnknown | ControlEffect.unknown | Inherently unobservable or non-state-mutating completion (.key, .select, .focus). Tolerated without modifying consecutiveNoEffectCount. Proceed to step k+1 |
| .noEffectObserved | ControlEffect.notObserved | State-mutating action (.press, .menu, .type) completed AX dispatch successfully (<250ms) but state unchanged post-action. Increment consecutiveNoEffectCount. Proceed if counter < maxConsecutiveNoEffect (2); halt chain if counter == 2 |
...
| Confirmation Tiers & Banner Safety | Single ControlPolicy.confirmationTier function | ControlPolicy.confirmationTier(step) maps actions to 3 tiers: (1) .physical: .quit, .clickAt, .key, and candidate/menu titles matching ControlPolicy.physicalConfirmationWords (pay, purchase, transfer, send, delete, erase, trash, empty, quit, reset, revoke, deactivate, uninstall, remove, clear, discard, order, book, publish, post, share, confirm, approve, sign out, log out, signout, logout, and any unclassified destructive stem evaluated with identical stemming and suffixes to isDestructiveControlTitle) require pointer click on Notch HUD confirmation banner with genuine HID hardware event state (checking kCGEventSourceStateHIDSystemState, rejecting synthetic CGEvents or AXPress self-approval, no keyboard Return/Space capture on non-key panel, avoiding focus loss and target default button activation). Confirmation banner for .physical tier uses a fixed 15.0s timer with no voice activity extension. (2) .spokenOrPhysical: benign review actions (unlisted menus, sensitive toggles, non-English target application locales, non-ASCII candidate titles, .type, .focus, .scroll, .open, .openFolder, .activate, .activateApplication, and words in ControlPolicy.spokenConfirmationWords: cancel, close, submit, reply, forward, archive, unsend) can be confirmed by spoken "confirm action" / "cancel action" ("approve action" / "abort action" accepted as spoken aliases) under active AEC with 5s silence / 15s max VAD timer, or by pointer click on banner; spoken confirmation words apply only if candidate title does not match any physical destructive stem (e.g. "resend" matches stem "send" and enforces .physical tier; "submit" remains .spokenOrPhysical); (3) .autoRun: returns .autoRun iff canAutoRun(step) evaluates true (requiring target application effective localization to begin with "en", ASCII-only candidate title, confidence >= 0.60, !step.requiresConfirmation, non-nil candidateTitle for .press/.select, and isDestructiveControlTitle == false for .press / .select, or allowlisted menu path without destructive segments); any step with confidence < 0.60, step.requiresConfirmation == true, non-English app localization, non-ASCII title, or nil candidateTitle falls back to confirmation review. Physical tier takes strict precedence over spoken tier on multi-word matches (e.g. "Cancel Order and Delete" is .physical). Dictation insertion into target app is suppressed while banner is active |
| Headless Action Confirmation | Headless callers require interactive human approval on Notch HUD | Automated CLI and socket calls cannot self-approve confirmation-gated actions (.physical or .spokenOrPhysical). Any gated action returns reviewRequired error JSON with stepId (15s TTL, single-use) bound strictly to the target identity: element actions (.press, .select, .focus, .clickAt, .scroll, .type, .key) bind to expectedFingerprint, processIdentifier, and windowTitle; application actions (.menu, .quit, .activate, .activateApplication) bind to bundleIdentifier and processIdentifier; path actions (.open, .openFolder) bind to destination URL. The caller awaits the socket response event or polls sayso control --status <stepId>; stepId is consumed (single-use) upon human approval on the Notch HUD banner (pointer click on banner for .physical; spoken confirm banner or click for .spokenOrPhysical) or upon expiration; execute() verifies bound target identity and records outcome in ControlSession.record; headless self-approval without human interaction is rejected fail-closed |
...
| Resilient Audit Storage | Journal schema changes and read errors must never wipe history | ControlAuditStore decodes entries individually, preserving raw JSON values (dictionaries, arrays, strings, numbers, nulls) for every entry. Writes control-audit.version sidecar strictly monotonically (never lowers schemaVersion on disk: newVersion = max(existingVersion, 1)) after journal is written and renamed, using atomic temp-file rename ordering; older builds encountering schemaVersion > 1 treat entries as raw JSON values, preserve existing schemaVersion, and append safely. Distinguishes missing file from read error (transient I/O, permissions); on read error in append, aborts fail-closed without overwriting disk journal, retaining in-memory state and displaying audit alert banner in Notch HUD. If dispatch succeeds but post-action append write fails, best-effort in-memory state is retained with non-fatal HUD warning banner without re-executing. Phase 1 per-entry decode deploys before Phase 2 (.menu). Raw entries count toward 500-entry cap and are preserved on re-save. Unrecognised entries (actions, effects, stepResult) decode as raw entries without backups; on whole-file JSON syntax failure, writes permanent initial backup control-audit.json.corrupt.initial and rotates up to 2 timestamped backups control-audit.json.corrupt-<timestamp> (capped at 3 backup files total) before fallback; if backup write fails, aborts append fail-closed without overwriting journal |
...
| Layer | Scope | Gate |
|---|---|---|
| Unit | Menu hierarchy step parsing | "menu File choose Save then click Confirm" splits on " choose " token delimiters (e.g. "menu Edit choose New Item" -> "Edit" / "New Item"; "menu Insert choose Item List choose Sort" -> "Insert" / "Item List" / "Sort") without ambiguity with child titles containing "item" or "open"; splits on " then " (case-insensitive) into discrete command steps. Empty path [] and depth > 3 reject fail-closed with invalidMenuPath. Spoken grammar uses "menu <Parent> choose <Child>"; CLI and automation socket use "menu File > Save" |
...
| Unit | Headless action approval | CLI and socket control calls return reviewRequired error JSON for gated steps; headless self-approval is barred fail-closed; execution requires interactive human confirmation on Notch HUD; stepId binds strictly to target identity (element fingerprint, bundle ID, or URL) including .key; outcome delivers over socket or status poll |
...
| Unit | Confirmation banner safety | Pointer click on Notch HUD confirmation banner with genuine HID hardware event state required for all .physical actions (.quit, .clickAt, .key, physical destructive words); synthetic CGEvents or AXPress self-approval rejected; spoken "confirm action" / "cancel action" ("approve action" / "abort action" accepted as spoken aliases) accepted under active AEC for benign review actions; bare words rejected |
...
| Unit | Resilient audit journal decoding | Corrupt or unknown action entries preserved without wiping 500-entry journal; raw JSON values (dictionaries, arrays, primitives) retained; transient read errors abort append without overwrite; initial corrupt backup control-audit.json.corrupt.initial and up to 2 timestamped backups (capped at 3 files) written on whole-file JSON syntax corruption; control-audit.version sidecar written strictly monotonically (never lowered) after journal write using atomic temp-file rename ordering |
