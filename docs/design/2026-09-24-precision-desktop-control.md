...
|---|---|---|---|
| ControlPlanner | SaysoCore | Sub-15ms closed-vocab intent parsing alongside LLM fallback | Extended: spoken grammar parses menu hierarchy using "then open" (e.g. "menu File then open Save"); CLI and automation socket use ">"; empty path [] and depth > 3 reject fail-closed with invalidMenuPath |
...
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw JSON values (dictionaries, arrays, strings, numbers, nulls) for every entry; append aborts fail-closed on read error without overwrite, retaining best-effort in-memory entries and displaying HUD alert banner; writes control-audit.version sidecar (schemaVersion: 1) using atomic write-and-rename ordering; future builds encountering schemaVersion > 1 treat entries as raw JSON values and append safely; if backup write fails on corrupt parse, aborts append fail-closed; ships ExecutionTelemetry, ControlEffect.timedOut, and ControlSessionStepResult.timedOut; retains initial corrupt backup and up to 2 timestamped backups (capped at 3 files) on whole-file JSON syntax failure (see Safety Invariants) |
...
| Menu Traversal | Spoken "menu File then open Save" or CLI "menu File > Save" | Cross-process AXMenuBar item press with confirmation banner |
...
└─────────┘  (stepId, 15s) └─────┬─────┘                     │
                                 ...
                                 ▼                           │
                           ┌───────────┐                     │
                           │ execute() │◀──human confirmed───┘
                           │ (verify   │
                           │  binding) │──▶ [Dispatch] ──▶ [Done]
                           └───────────┘
```
...
| Menu Traversal Execution Timing | Menu traversal executes inside execute() after confirmation | Confirmation gate statically inspects menu path against allowlist and destructive stems without opening menus. AXMenuBar traversal (depth ≤3) runs inside execute() post-approval, preventing open menus from shifting focusedRole/focusedValue or timing out during user confirmation. If traversal times out, performs kAXCancelAction or kAXPressAction on opened parent menu item to dismiss menus (never synthesizes raw CGEvent Escape keys without confirmation); if dismissal fails, shows non-fatal HUD alert "Menu Left Open" and logs menuDismissalFailed to audit |
...
| Layer | Scope | Gate |
|---|---|---|
| Unit | Menu hierarchy step parsing | "menu File then open Save then click Confirm" splits on " then open " token delimiters; titles containing "item" (e.g. "menu Edit then open New Item" -> "Edit" / "New Item"; "menu Insert then open Item List then open Sort" -> "Insert" / "Item List" / "Sort") parse cleanly without ambiguity with child titles; splits on " then " (case-insensitive) into discrete steps. Empty path [] and depth > 3 reject fail-closed with invalidMenuPath. Spoken grammar uses "menu <Parent> then open <Child>"; CLI and automation socket use "menu File > Save" |
...
| Unit | Headless action approval | CLI and socket control calls return reviewRequired error JSON for gated steps; headless self-approval is barred fail-closed; execution requires interactive human confirmation on Notch HUD; stepId binds strictly to target identity (element fingerprint, bundle ID, or URL) |
...
| Unit | Menu traversal timeout dismisses menu via AX | Menu traversal timeout triggers kAXCancelAction or kAXPressAction on opened parent menu; zero CGEvent Escape keys synthesized |
...
| Unit | Resilient audit journal decoding | Corrupt or unknown action entries preserved without wiping 500-entry journal; raw JSON values (dictionaries, arrays, primitives) retained; transient read errors abort append without overwrite; initial corrupt backup control-audit.json.corrupt.initial and up to 2 timestamped backups (capped at 3 files) written on whole-file JSON syntax corruption; control-audit.version sidecar written using atomic temp-file rename ordering |
