...
**Why A:** Integrates jev-use closed-vocabulary fast-path planner patterns into existing SaysoCore types, leveraging bounded cross-process AX queries and instant badge overlays while maintaining full safety guarantees. Residual Risk: (1) macOS accessibility architecture allows any local process with system-wide Accessibility trust (AXIsProcessTrusted) to post events to AppKit windows. This risk is accepted because obtaining Accessibility trust already grants arbitrary desktop control. (2) Voice Control audio injection: an unprivileged local headless caller running TTS (`say`) cannot self-approve gated actions because headless confirmation banners display a dynamic one-time verification token (e.g. "Confirm [47]") visible only on screen and strictly omitted from caller API responses. The physical click or spoken approval under Voice Control requires matching this dynamic token ("confirm 47"), preventing blind self-approval via scripted TTS.
...
|---|---|---|
| Auto-Run Gate | canAutoRun(step) == true | Target app effective localization begins with "en", ASCII-only candidate title, confidence >= 0.60, !step.requiresConfirmation, non-nil candidateTitle for .press/.select, and isDestructiveControlTitle == false for .press/.select (or safeMenuAllowlist path without destructive segments); any failing condition falls back to confirmation review |
| Action Tier Mapping | ControlPolicy.confirmationTier(step) | Exhaustively maps all 13 DesktopAction cases: (1) .physical: .quit, .clickAt, mutating/destructive .key; .press, .select, .menu if matching physical stems/words or unclassified destructive stems. (2) .spokenOrPhysical: navigation .key; benign .press, .select, .menu; .type, .focus, .scroll, .open, .openFolder, .activate, .activateApplication. (3) .autoRun: returns .autoRun iff canAutoRun(step) evaluates true. Physical tier takes strict precedence over spoken tier on multi-word matches |
...
| Destructive Stems Partition | Exact partition of ControlPolicy.destructiveStems (28 stems) | Strict partition into two disjoint sets with empty leftover: (1) Spoken stems (7 stems): cancel, clos, submi, repl, forward, archiv, unsend. (2) Physical stems (21 stems + extended tokens): delet, remov, clear, empt, eras, uninstall, revok, deactivat, reset, send, resend, post, shar, publish, pay, purchas, order, transfer, book, confirm, approv, plus extended tokens sign out, log out, signout, logout, disconnect, leave, quit |
| Headless Anti-Injection Token | Dynamic verification token on Notch HUD | For headless CLI/socket callers, Notch HUD banner displays a dynamic 2-digit verification token (e.g. "Confirm [47]"); token is strictly omitted from caller reviewRequired JSON response (which carries only stepId); human confirmation requires clicking tokenized banner or speaking tokenized phrase under Voice Control ("confirm 47"), preventing blind self-approval via scripted local TTS (say "confirm action") |
...
| Unit | Headless action approval | CLI and socket control calls return reviewRequired error JSON for gated steps (omitting dynamic verification token); headless self-approval is barred fail-closed; execution requires interactive human confirmation matching dynamic token on Notch HUD; spoken confirmation barred without dynamic token to block local TTS injection; interactive commands preempt pending headless slots; stepId binds strictly to target identity |
