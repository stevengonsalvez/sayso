...
└───────────┘               └────────────┬────────────┘
           ...
           └─────────┬─────────┘           │ (Dictation suppressed; 5s/15s VAD timeout, cancel on expire)
                     ...
                     ▼─────────────────────┘
           ┌──────────────────────────────────────────────┐
           │ execute() Verification & Pre-flight          │
           │ • Auto-run: target app PID & window verify   │
           │ • Post-confirm: fresh capture & fingerprint  │
...
| ControlPolicy | SaysoCore | Fail-closed destructive classification and review gating | Formalized invariant: compiler eliminates default: fallbacks; DesktopAction.isDestructive for .menu checks isDestructiveControlTitle(leaf) for intrinsic reporting; ControlPolicy.requiresConfirmation gates unlisted menus (!safeMenuAllowlist.contains(path)) or destructive titles; existing ControlPolicy.requiresConfirmation classifies .press and .select by candidate title via isDestructiveControlTitle to allow benign buttons to auto-run; .clickAt and .key remain blanket gated; destructive check wins |
| AXDesktopController | SaysoCore | Semantic AX action dispatch and confirmation-gated pointer clicks | Extended: DesktopAction.menu(path:expectedApplicationTarget:) and 250ms timeout; runs post-action observation after cannotComplete/timeout; timed-out dispatches are non-retryable; on any AX timeout or cannotComplete without diff (regardless of elapsed time), execute() returns ControlObservationResult with effect = .timedOut rather than throwing an un-audited error; menu traversal, leaf AXTitle re-check, and pre-flight run inside execute() after confirmation |
...
| ControlSession | SaysoCore | Multi-step lifecycle, budget enforcement, and state transitions | Extended: record(.actionFailed) and record(.timedOut) call finish(.failed) directly; halts fail-closed immediately on single .timedOut or on 2 consecutive .noEffectObserved; existing fail() call sites in SaysoNotchApp and tests updated accordingly |
| NotchHUD | SaysoNotch | Visual reticle, badge overlay, and telemetry breakdown | Extended: spatial badge rendering on nonactivating panel; high-risk actions (.clickAt, .key, pay/send/delete) require physical Return/click; benign review actions allow spoken confirm/cancel banner with 5s/15s VAD timeout under active AEC |
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw dictionary for every entry; pre-dispatch stat/descriptor read probe aborts and blocks action on read errors without overwrite; if backup write fails on corrupt parse, aborts append fail-closed; ships ExecutionTelemetry, ControlEffect.timedOut, and ControlSessionStepResult.timedOut; retains initial corrupt backup and up to 2 timestamped backups (capped at 3 files) on whole-file JSON syntax failure (see Safety Invariants) |
...
| Active Target & AX Capture | 40ms (2x 20ms p50) | 1500ms (2x 750ms) | Fast-path auto-run (~71ms p50) reuses plan-time capture #1 for execution while performing a cheap (<1ms) frontmost window and PID identity check; capture #2 is the post-dispatch verification capture (20ms p50); 2 captures total (40ms p50). Confirmation-gated flows run a full fresh capture to verify target window fingerprint. 1500ms worst case represents 2 captures capped by 500ms soft clock check plus single 250ms in-flight call overrun each |
| Pre-flight & Audit Read Probe | 4ms (2x 2ms) | 500ms (2x 250ms) | isEnabled and isProtected attribute checks (2 calls x 250ms timeout) plus sub-millisecond stat/descriptor audit store readability pre-flight probe |
| Semantic Action Dispatch | 4ms | 250ms | AXUIElementPerformAction (bounded by 250ms messaging timeout) |
| Post-assert Diff Calculation | 15ms (initial check) | 1450ms | In-memory diff calculation evaluated against verification capture #2 (not a separate AX pass); immediate check at 0ms. If unverified, up to 7 subsequent polling re-captures spaced by 125ms intervals up to attempt cap of 8 attempts (~875ms interval delays); 1200ms monotonic soft ceiling backstop bounds worst-case slow AX queries with at most one 250ms in-flight call overrun = 1450ms cutoff |
...
```
[Voice Cmd] ──Plan (8ms)──▶ [Capture #1 (20ms)] ──Pre-check (4ms)──▶ [AX Dispatch (4ms)] ──Verification Capture #2 & Diff (20ms + 15ms)──▶ [Done (~71ms target)]
```
...
| Confirmation Banner Safety | Tiered physical and spoken confirmation | High-risk destructive actions (pay, purchase, transfer, send, delete, erase, quit, reset) and .clickAt pointer clicks require physical Return key or pointer click on HUD banner. Benign review actions (unlisted menus, sensitive toggles) allow spoken "confirm action" / "approve action" (and "cancel action" / "abort action") under active system AEC voice processing. Bare words ("yes", "no", "cancel") are rejected on open mic. Dictation insertion into target app is suppressed while banner is active |
| Headless Action Confirmation | Headless callers require explicit approval flag for destructive actions | For automated CLI and socket calls, any action requiring review fails closed and returns reviewRequired error JSON unless caller passes explicit --approve flag (or approved: true in IPC payload) |
...
| Fail-Closed Menu Policy | Explicit .menu case; allowlist check must be non-destructive | safeMenuAllowlist.contains(path) && !isDestructiveControlTitle(leaf); empty path [] or depth > 3 rejects fail-closed with invalidMenuPath |
| Menu Destructive Classification | Explicit .menu case in DesktopAction.isDestructive | In DesktopAction.isDestructive, .menu(path: _, expectedApplicationTarget: _) evaluates isDestructiveControlTitle(leaf) for intrinsic reporting; ControlPolicy.requiresConfirmation evaluates !safeMenuAllowlist.contains(path) || isDestructiveControlTitle(leaf) for execution gating, eliminating default: false fallbacks |
...
| Resilient Audit Storage | Journal schema changes and read errors must never wipe history | ControlAuditStore decodes entries individually, preserving raw dictionary for every entry. Distinguishes missing file from read error (transient I/O, permissions); pre-dispatch audit readability probe (stat/descriptor check) aborts dispatch fail-closed on read/permission error without overwriting journal, blocking the action and displaying an audit alert banner in Notch HUD. If dispatch succeeds but post-action append fails, best-effort in-memory state is retained with non-fatal HUD warning banner without re-executing. Phase 1 per-entry decode deploys before Phase 2 (.menu). Raw entries count toward 500-entry cap and are preserved on re-save. Unrecognised entries (actions, effects, stepResult) decode as raw dictionaries without backups; on whole-file JSON syntax failure, writes permanent initial backup control-audit.json.corrupt.initial and rotates up to 2 timestamped backups control-audit.json.corrupt-<timestamp> (capped at 3 backup files total) before fallback; if backup write fails, aborts append fail-closed without overwriting journal |
...
| Layer | Scope | Gate |
|---|---|---|
| Unit | Menu hierarchy step parsing | "menu File item Save then click Confirm" splits on " then " (case-insensitive) into 2 steps while preserving "item" hierarchy within menu path. Empty path [] and depth > 3 reject fail-closed with invalidMenuPath. Spoken grammar uses "menu <Parent> item <Child>" (e.g. "menu File item Save"); CLI and automation socket use "menu File > Save". Segment quoting is scoped to CLI and automation socket; spoken input uses unquoted token splitting and cannot generate quote characters |
...
| Unit | Headless destructive action approval | CLI and socket control calls without --approve return reviewRequired error JSON for destructive steps; --approve executes action |
...
| Unit | Confirmation banner safety | Physical Return key or pointer click required for high-risk actions (.clickAt, pay/send/delete); spoken "confirm action" / "approve action" / "cancel action" / "abort action" accepted under active AEC for benign review actions; bare words rejected |
...
| Unit | Audit read error blocks action execution | Pre-dispatch audit stat/read probe in ControlAuditStore aborts dispatch fail-closed on transient read error, blocking action execution and displaying audit alert in Notch HUD; post-dispatch append failure retains best-effort in-memory state with non-fatal HUD warning banner |
...
| Benchmark | Spoken confirmation acoustic rejection | Measure spoken confirmation trigger accuracy under active system AEC against background speaker audio and media playback containing spoken confirmation phrase; acceptance gate: false accepts = 0 across 300 background playback trials |
