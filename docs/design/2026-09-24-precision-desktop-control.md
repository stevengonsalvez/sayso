...
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw dictionary for every entry; ships ExecutionTelemetry; writes single control-audit.json.corrupt on whole-file JSON syntax failure without overwriting |
...
| Post-assert Diff | 15ms (initial check) | 1450ms | Immediate check at 0ms sleep; 1200ms monotonic soft deadline ceiling (bounded by at most one 250ms in-flight AX call overrun = 1450ms worst-case abort). Initiates attempts spaced by 125ms intervals with 100ms soft check per attempt, terminating at min(8 attempts, 1200ms ceiling) (~5-8 attempts) |
...
| .effectUnknown | ControlEffect.unknown, immediate cannotComplete | Benign unobservable completion (.key, .select, or modal open in <250ms). Tolerated without modifying consecutiveNoEffectCount. Proceed to step k+1 |
...
| .timedOut | AX call timeout (elapsed >= 250ms) | Call messaging timeout. Never retried fail-closed. Increment consecutiveNoEffectCount; 2 consecutive unverified (.noEffectObserved or .timedOut) halt session |
...
| AX Timeout Detection | Timeouts map to .timedOut; never retried | Elapsed call time >= 250ms with kAXErrorCannotComplete maps to .timedOut, incrementing consecutiveNoEffectCount (2 consecutive unverified outcomes halt session fail-closed). Timed-out actions are non-retryable to prevent double-firing in target app. Immediate (<250ms) cannotComplete or unobservable actions (.key, .select) map to .effectUnknown and are tolerated without altering counter |
...
| Unit | Consecutive AX timeouts halt session | AX calls timing out (elapsed >= 250ms) map to .timedOut, are non-retryable, and 2 consecutive timeouts halt multi-step chain fail-closed |
| Unit | Spatial row-bucket badge ordering | Badges 1..N order by anchored row (Y ±4pt threshold), then X |
| Unit | Resilient audit journal decoding | Corrupt or unknown action entries preserved without wiping 500-entry journal; single backup control-audit.json.corrupt written without overwriting only on whole-file JSON syntax corruption |
