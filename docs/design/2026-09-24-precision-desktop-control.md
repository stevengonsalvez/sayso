...
| NotchHUD | SaysoNotch | Visual reticle, badge overlay, and telemetry breakdown | Extended: spatial badge rendering on nonactivating panel; intercepts physical Return/Space via local NSEvent monitor without focus loss; high-risk actions (.clickAt, .key, pay/send/delete) require physical Return/click; benign review actions allow spoken confirm/cancel banner with 5s/15s VAD timeout under active AEC |
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw dictionary for every entry; append aborts fail-closed on read error without overwrite, retaining best-effort in-memory entries and displaying HUD alert banner; if backup write fails on corrupt parse, aborts append fail-closed; ships ExecutionTelemetry, ControlEffect.timedOut, and ControlSessionStepResult.timedOut; retains initial corrupt backup and up to 2 timestamped backups (capped at 3 files) on whole-file JSON syntax failure (see Safety Invariants) |
...
| Active Target & AX Capture | 40ms (2x 20ms p50) | 1500ms (2x 750ms) | Fresh pre-dispatch AX capture #1 (20ms p50) checks expectedFingerprint equality (TOCTOU guard) and frontmost window/PID on both auto-run and confirmed flows; post-dispatch verification capture #2 (20ms p50) captures after-state; 2 captures total (40ms p50). 1500ms worst case represents 2 captures capped by 500ms soft clock check plus single 250ms in-flight call overrun each |
| Pre-flight Attribute Check | 4ms (2x 2ms) | 500ms (2x 250ms) | isEnabled and isProtected attribute checks (2 calls x 250ms timeout) on target element |
...
| Confirmation Tiers & Banner Safety | Single ControlPolicy.confirmationTier function | ControlPolicy.confirmationTier(step) maps actions to 3 tiers: (1) .physical: .clickAt, .key, and candidate/menu titles matching high-risk destructive words (pay, purchase, transfer, send, delete, erase, trash, empty, quit, reset, revoke) require physical Return/Space key (captured via local NSEvent monitor on NotchHUD without stealing focus) or pointer click on banner; (2) .spokenOrPhysical: benign review actions (unlisted menus, sensitive toggles, cancel, close, submit, post) can be confirmed by spoken "confirm action" / "approve action" under active AEC or by physical Return/click; (3) .autoRun: benign button .press / .select with isDestructiveControlTitle == false and allowlisted menus. Dictation insertion into target app is suppressed while banner is active |
| Headless Action Confirmation | Headless callers require step-bound approval; high-risk barred headless | For automated CLI and socket calls, high-risk actions (.clickAt, pay/send/delete) are barred headless and return reviewRequired error JSON directing user to Notch HUD. Benign review actions require explicit step-bound approval specifying stepId previously returned in reviewRequired response (sayso control approve --step-id <UUID>), preventing blind batch approval |
...
| Bounded AX Timeout | Cross-process AX queries cannot hang UI | AXUIElementSetMessagingTimeout 0.25s per call set on frontmost AXUIElement application element (covering all descendant queries); 500ms soft clock check |
...
| Unit | Headless destructive action approval | CLI and socket control calls return reviewRequired error JSON for destructive steps; high-risk actions barred headless; benign review actions execute only with explicit step-bound approval (--approve --step-id <UUID>) |
...
| Unit | Confirmation banner safety | Physical Return/Space key (local NSEvent monitor) or pointer click required for high-risk actions (.clickAt, .key, pay/send/delete); spoken "confirm action" / "approve action" / "cancel action" / "abort action" accepted under active AEC for benign review actions; bare words rejected |
