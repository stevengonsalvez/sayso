...
|---|---|---|---|
| ControlPlanner | SaysoCore | Sub-15ms closed-vocab intent parsing alongside LLM fallback | Extended: spoken grammar parses menu hierarchy using "choose" (e.g. "menu File choose Save"); CLI and automation socket use ">"; path length < 2 or > 3 rejects fail-closed with invalidMenuPath (e.g. ["File"] rejected) |
...
| NotchHUD | SaysoNotch | Visual reticle, badge overlay, and telemetry breakdown | Extended: spatial badge rendering on nonactivating panel; all .physical actions (.quit, .clickAt, .key, physical destructive words) require pointer click on HUD banner with genuine user interaction (accepting macOS Accessibility assistive tools: Voice Control, Switch Control, Dwell Click; automated background CLI/socket programmatic self-approval rejected fail-closed, no key capture on non-key panel, avoiding focus stealing and target app default button activation); benign review actions allow spoken confirm/cancel banner with 5s/15s VAD timeout under active AEC or pointer click |
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw JSON values (dictionaries, arrays, strings, numbers, nulls) for every entry guarantees forward and backward schema compatibility without external sidecars; append aborts fail-closed on read error without overwrite, retaining best-effort in-memory entries and displaying HUD alert banner; ships ExecutionTelemetry, ControlEffect.timedOut, and ControlSessionStepResult.timedOut; retains initial corrupt backup and up to 2 timestamped backups (capped at 3 files) on whole-file JSON syntax failure (see Safety Invariants) |
...
| Fail-Closed Menu Policy | Explicit .menu case; allowlist check must be non-destructive | path.count >= 2 && path.count <= 3 (requiring parent and child leaf, rejecting single segment e.g. ["File"] or depth > 3 fail-closed with invalidMenuPath); safeMenuAllowlist.contains(path) && !path.contains(where: { isDestructiveControlTitle($0) }) |
