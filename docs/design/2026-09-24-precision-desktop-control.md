...
| ControlAuditStore | SaysoCore | Resilient persistent recording of newest 500 entries | Extended: Phase 1 per-entry decode via JSONSerialization preserving raw dictionary for every entry; non-object primitive entries (strings, numbers, nulls) decode into raw entries and are preserved without wiping; append aborts fail-closed on read error without overwrite, retaining best-effort in-memory entries and displaying HUD alert banner; writes control-audit.version sidecar (schemaVersion: 1) so legacy builds reject newer files without wiping; if backup write fails on corrupt parse, aborts append fail-closed; ships ExecutionTelemetry, ControlEffect.timedOut, and ControlSessionStepResult.timedOut; retains initial corrupt backup and up to 2 timestamped backups (capped at 3 files) on whole-file JSON syntax failure (see Safety Invariants) |
...
| Menu Traversal Execution Timing | Menu traversal executes inside execute() after confirmation | Confirmation gate statically inspects menu path against allowlist and destructive stems without opening menus. AXMenuBar traversal (depth ≤3) runs inside execute() post-approval, preventing open menus from shifting focusedRole/focusedValue or timing out during user confirmation. If traversal times out, performs kAXCancelAction or kAXPressAction on opened parent menu item to dismiss menus (never synthesizes raw CGEvent Escape keys without confirmation) |
...
| Layer | Scope | Gate |
|---|---|---|
| Unit | Menu hierarchy step parsing | "menu File item Save then click Confirm" splits on whole-word "item" token delimiters; titles containing "Item" (e.g. "menu Edit item New Item" -> "Edit" / "New Item"; "menu Insert item Item List item Sort" -> "Insert" / "Item List" / "Sort") parse cleanly without truncating child title; splits on " then " (case-insensitive) into discrete steps. Empty path [] and depth > 3 reject fail-closed with invalidMenuPath. Spoken grammar uses "menu <Parent> item <Child>"; CLI and automation socket use "menu File > Save" |
...
| Unit | Resilient audit journal decoding | Corrupt or unknown action entries preserved without wiping 500-entry journal; transient read errors abort append without overwrite; initial corrupt backup control-audit.json.corrupt.initial and up to 2 timestamped backups (capped at 3 files) written on whole-file JSON syntax corruption; control-audit.version sidecar written |
